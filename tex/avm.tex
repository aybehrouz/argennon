...
\begin{document}
    ...
    \note{Only public methods can be invoked by \texttt{invoke\_external} instruction.}
    ...
    \note{The reason behind this paged design is that the heap is usually persisted using a block device. A heap with
    ...
    \note{In the future, we may need to add special instructions for invoking interface and virtual methods...}
    ...
    \note{Validators do not need to write the modified pages back to ZK-EDB servers. ZK-EDBs will receive the new
