...
\begin{document}
    ...
    To mitigate this problem, for calculating a user's stake at the round \(n\), instead of using the raw ALGO
    balance, we use the minimum of a \emph{trust value} the system has calculated for the user and the user's
    ALGO balance:
    \[
        Stake_{user,n} = \min (Balance_{user,n}, Trust_{user,n})
    \]
    For estimating the value of \(Trust_{user,n}\) we use the following formula:
    \[
        Trust_{user,n} = \max (M_n, \beta Balance_{user,n})
    \]
    Where \(M_n\) is the exponential moving average of the user's ALGO balance at the round \(n\) and \(\beta\) is a
    ...
    than a new user with a higher balance. An attacker who has obtained a large amount of ALGOs, also needs to hold
