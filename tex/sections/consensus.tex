...
validators with the index $h+1$ modulo $m$. When a
...
h_v+k+1$. The parent of the fork block with height $h_v+i$ will be the block $h_v+i-1$ of the main chain. Then the
reserve committee will wait until the fork block with height $h_v+k+1$ receives a certificate form validators. The
reserve committee then will continue the normal chain after that fork block and the fork block with height $h_v+k+1$
...
of all the assemblies of validators are honest, for a fork block with height $h_f$ we have:
\begin{itemize}
...
block after any of the fork blocks with heights less than $h_0+k$, that normal block can not receive a certificate from
validators.
...
\begin{itemize}
    ...
    proposal.
    \item if the user has \textbf{not} final-voted any value, if he sees more than 2/3 $r$-votes for a proposal $p$, he
    final-votes $p$.
    \item if he sees more than 2/3 final-votes for a proposal, he goes to the confirmation phase for $p$.
    \item if $clock > r \cdot \lambda$ and $r < k$ the user goes to the round $r + 1$ and if $r = k$ user starts a
    new agreement session.
\end{itemize}

\paragraph{Confirmation Phase for Proposal $p$:}
\begin{itemize}
    \item the user c-votes $p$.
...
$p_2$. In the first round all users $1$-vote for $p_1$, but only $s/3 - m$ stake sees more than $2/3$ $1$-votes. At the
next round all the remaining stake $2$-votes for $p_2$. Again, only $s/3 - m$ stake sees more than $2/3$
...
Consequently, after $k$ rounds the session will end without an agreement and a new session will be started.
