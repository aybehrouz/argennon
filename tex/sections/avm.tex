...
\[
    (A^{(1)},A^{(2)},A^{(3)},\dots)\ ,
\]
where \(A^{(i)}=(0.a_{1}a_{2}\dots a_{i})_\beta\), and we have \(A^{(i)} \leq A^{(i+1)}\).
...
Every heap chunk has a size: \texttt{chunkSize} and a size upper bound: \texttt{sizeUpperBound}. The value of
\texttt{chunkSize} can be determined uniquely at
the start of
every execution session, and it may be updated by the owner application during the session like a normal memory
location. On the other hand,
the value of \texttt{sizeUpperBound} is constant for every block of the Argennon blockchain and is proposed by the block
proposer. This value indicates the upper bound of \texttt{chunkSize} for a specific chunk during the execution of all
requests of a block, and it can be used by validators for memory allocation of chunks during block validation.
...
defined on a chunk. It has an offset and a size and determines the accessible memory locations inside the chunk.
Multiple access blocks can be defined on a single chunk, but they must be non overlapping.
Access blocks are byte addressable and can have different access types:
...
\begin{itemize}
    ...
    \item \texttt{additive}: only allows additive operations. These operations are different types of mathematical
    addition without any overflow checking. Note that the content of these access blocks cannot be read.
\end{itemize}
...
validating a block in the same chunk and the data needed in a single execution session in the same access block.
...
preprocessing phase of the request execution, and any type of
