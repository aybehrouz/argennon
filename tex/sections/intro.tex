...
Each minted block gets certified by all members of the delegates committee and after that it required to get approved
by its corresponding validator committee.
...
In Argennon, a block is considered final after its \textbf{next} block gets certified by \textbf{both} delegates and
...
More formally, each CI statement has the form $\tau(\mathbb{S}, \mathbb{R}, \hat{\mathbb{S}})$, and states that
$\hat{\mathbb{S}}$ is the commitment of the next state after executing an ordered list of external requests with commitment
$\mathbb{R}$ on a state with commitment $\mathbb{S}$.
...
of these CI statements (STARK/SNARK) from the Argennon cloud and efficiently validate a block.
